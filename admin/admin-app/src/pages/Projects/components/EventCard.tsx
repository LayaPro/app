...
export const EventCard: React.FC<EventCardProps> = ({ event, index, teamMemberOptions, eventTypeOptions, onRemove, onEdit }) => {
  ...
  const isIncomplete = event.fromProposal && (!event.fromDate || !event.fromTime);

  return (
    <div className={`${styles.eventCard} ${isIncomplete ? styles.incompleteEvent : ''}`}>
      <div className={styles.eventHeader}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div className={styles.eventName}>{event.eventName}</div>
          {event.fromProposal && (
            <span className={styles.fromProposalBadge}>From Proposal</span>
          )}
          {isIncomplete && (
            <span className={styles.incompleteWarning}>⚠️ Incomplete</span>
          )}
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            type="button"
            className={`${styles.editButton} ${isIncomplete ? styles.editButtonHighlight : ''}`}
