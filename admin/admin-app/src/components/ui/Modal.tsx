  ...
  useEffect(() => {
    if (isOpen) {
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          setIsAnimating(true);
          // Focus the first focusable element (prioritize form controls over buttons)
          setTimeout(() => {
            const firstFocusable = modalRef.current?.querySelector<HTMLElement>(
              'input:not([type="hidden"]), select, textarea, button[class*="select"]'
            );
            if (firstFocusable) {
              firstFocusable.focus();
            } else {
              // If no focusable elements, ensure scroll is at top
  ...
  return createPortal(
    <div className={`${styles.overlay} ${isAnimating ? styles.open : ''}`}>
      ...
      >
        ...
        <div className={`${styles.content} ${!title ? styles.noHeader : ''}`} onFocus={handleFocus}>
          {children}
        </div>
      </div>
    </div>,
